...
in Julia with the following features:
 ...
 - Most common types of materials and radiation sources are provided, but 
additional ones can be added by the user.  
 ...
 - A grid cloner that duplicates (implicitly) the scene in a grid to
approximate large canopies.   
 - A Russian roulette mechanism to reduce the number of iterations per ray
needed while avoiding introducing biases in the computations.  
...
(this is `maxiter` in [`RTSettings()`](../../api/RayTracer.md#VPL.RayTracing.Source)). 
...
(see [documentation](../Turtle)) and the radiation sources (see below) which
...
documentation on [`Source`](../../api/RayTracer.md#VPL.RayTracing.Source) for more details.
...
See documentation on [`DirectionalSource`](../../api/RayTracer.md#VPL.RayTracing.DirectionalSource) 
for more details.
