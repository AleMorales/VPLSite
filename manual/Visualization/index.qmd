...
See section on [Graphs](../Graphs)) and how to access nodes directly from their
...
primitive constructor, see section on [Primitives](../Primitives)). 

When a mesh is being rendered directly, a default color will be used for the 
mesh that the user can override (see API documentation for [`render()`](../../api/Render.md#VPL.Render.render)).
...
`Scene` object. As shown in documentation for [Scenes](../Turtle), the `feed`
