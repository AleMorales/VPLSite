...
rule = createRule(Cell, lhs = transfer, rhs = (context, father) -> Cell(node(father).state), captures = true)
axiom = Cell(1) + Cell(0) + Cell(0)
pop = createOrganism(axiom, rule)
```

In the original state defined by the axiom, only the first node contains a state of 1

```{julia}
[n.data.state for n in values(pop.nodes)]
```
