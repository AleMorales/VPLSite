...
*Node*: What is left after a meristem produces a new organ. They contain no data or geometry (so also a point) but are required to keep the branching structure of the tree as well as connecting leaves. Note that we cannot call this data type `Node` as that is a type already defined by VPL.
