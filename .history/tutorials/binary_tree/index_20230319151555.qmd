...
module btree
    ...
    Base.@kwdef struct Leaf <: VPL.Node
        length::Float64 = 0.1 # Leaves are 10 cm long
...
function prob_break(bud)
    ...
    check, steps = hasDescendent(node, condition = n -> data(n) isa btree.Meristem)
...
```{julia}
axiom = btree.Internode() + btree.Meristem()
```
...
```{julia}
tree = Graph(axiom = axiom, rules = (meristem_rule, branch_rule), vars = btree.treeparams())
```
