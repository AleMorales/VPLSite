...
*Leaf*: These are the nodes associated to leaves in the tree. They are represented by ellipses.
...
As always, the 3D structure and the color of each type of node are implemented with the `feed!` method. In this case, only the internodes andleaves have a 3D representation, so these methods are defined for this type only.
