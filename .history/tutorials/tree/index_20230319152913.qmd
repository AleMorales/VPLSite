...
```{julia}
axiom = tree.Internode() + tree.Meristem()
```
...
```{julia}
tree = Graph(axiom = axiom, rules = (meristem_rule, branch_rule), vars = tree.treeparams())
```
...
```{julia}
getInternode = Query(tree.Internode)
```
