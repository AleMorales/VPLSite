...
As always, the 3D structure and the color of each type of node are implemented with the `feedgeom!` method. In this case, only the internodes have a 3D representation, so these methods are defined for this type only.
