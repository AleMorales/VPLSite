...
The first step is to create a scene with the trees and the light sources. The
geometry of the scene is generated from the `feedgeom!()` methods, just like for
3D rendering. However, instead of assigning a color to the different objects, we
need to assign optical properties for the different wavelengths, via a `Material`
object. The `feedmaterial!()` method is used to do so. The `Material` object not
only provides the optical property but also stores the absorbed PAR. Hence it is
important to store these objects in the graph itself (see above for the definition
of `Internode`). Hence, the `feedmaterial!()` method will generally just pass
the material object of each node to the turtle (note that this would be
a `RTTurtle`, not a `MTurtle` or `GLTurtle`, the `RT` standing for ray tracing).

```{julia}
function VPL.feedmaterial!(turtle::RTTurtle, i::rbtree.Internode)
    feedmaterial!(turtle, i.material)
    return nothing
end
```

The scene can be created from the `forest` object by simply calling `RTScene(forest)` 
